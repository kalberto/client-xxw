var lead_mixin = {
  ...
  methods: {
    set_localstorage: function set_localstorage() {
      if (!(document.referrer == null || document.referrer == undefined || document.referrer == "")) {
        if (document.referrer.includes('google.com')) {
          if (typeof document.URL.split('?gclid=')[1] === "undefined") localStorage.setItem("referrer", 'google-organico');else localStorage.setItem("referrer", 'google-adwords');
        } else {
    ...
    send_lead: function send_lead(origem, endpoint, var_formulario) {
      ...
      Object.getOwnPropertyNames(this.lead[var_formulario]).forEach(function (item) {
        if (item == "file") app.$data.lead[var_formulario][item].forEach(function (item2) {
          data.append('file', item2);
        });else data.append(item, app.$data.lead[var_formulario][item]);
      });
